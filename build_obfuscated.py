...
MODULES = ['license_verify.py', 'engine.py', 'deepl.py', 'main.py']
...
def build():
    ...
    for mod_file in MODULES:
        mod_path = os.path.join(SRC_DIR, mod_file)
        if os.path.isfile(mod_path):
            with open(mod_path, 'r', encoding='utf-8') as f:
                source = f.read()
            encrypted = _xor_encrypt(source.encode('utf-8'), enc_key)
            mod_name = mod_file.replace('.py', '')
            module_blobs[mod_name] = base64.b64encode(encrypted).decode('ascii')
            print(f'  [+] src/{mod_file} -> {len(encrypted)} bytes')
    ...
    fl = f'_{random.choice("abcdefg")}{random.randint(100,999)}'
...
import tkinter.messagebox as {mv[6]}
...
def {fl}():
    valid, msg = {mv[4]}.modules['license_verify'].check_license()
    if not valid:
        try:
            {mv[6]}.showerror("SeedCopy License", msg + "\\n\\nContact: gongjumao123")
        except:
...
'''
    ...
    with open(out_path, 'w', encoding='utf-8') as f:
        f.write(guard)
    print(f'[_loader.py] Generated ({len(guard)} chars, {len(module_blobs)} modules)')
    return out_path
