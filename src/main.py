...
from license_verify import get_license_info
...
class App:
    ...
    def _build_ui(self):
        ...
        if info.get('status') == 'valid':
            rd = info.get('remaining_days', -1)
            exp = '永久有效' if rd < 0 else f'剩余 {rd} 天'
            lic_text = f"已授权: {info.get('customer', '')} | {exp}"
            lic_color = '#27ae60'
        else:
            lic_text = info.get('message', '未授权')
            lic_color = '#e74c3c'
